...
def chat(request: ChatRequest):
    ...
    if len(history) >= 20:
        try:
            ...
            print(f"[truncate] 已摘要前8轮对话")
        except Exception as e:
            print(f"[truncate] 摘要失败: {e}")

    # Mem0 检索用户记忆
    memory_context = search_user_memory(session_id, request.message)
    ...
    if cached:
        ...
        print(f"[cache] ✅ API层命中 intent={intent_guess}")
    ...
    if should_escalate:
        ...
        print(f"[escalation] 升级到人工: intent={intent} ticket={ticket_id}")
    ...
    save_user_memory(session_id, recent)
